...
_lsp_sqlesc() { printf '%s' "$1" | sed "s/'/''/g"; }

_lsp_query_blob() {
    _lsp_check_schema || return 1
    _lsp_query \
      "SELECT hex(data) FROM module_configs \
         WHERE module_pkg_name='$(_lsp_sqlesc "$1")' \
           AND user_id=$LSP_UID \
           AND \"group_name\"='$(_lsp_sqlesc "$2")' \
           AND key_name='$(_lsp_sqlesc "$3")';"
}
...
_lsp_write_blob() {
    ...
    _lsp_query \
      "INSERT OR REPLACE INTO module_configs \
       ...
       VALUES ('$(_lsp_sqlesc "$1")', $LSP_UID, '$(_lsp_sqlesc "$2")', '$(_lsp_sqlesc "$3")', x'$4');"
}
